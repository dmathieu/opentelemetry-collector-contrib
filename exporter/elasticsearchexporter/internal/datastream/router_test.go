...
func createRouteTests(dsType string) []routeTestCase {
	renderWantRoute := func(dsType, dsDataset string, otel bool) string {
		if otel {
			return fmt.Sprintf("%s-%s.otel-%s", dsType, dsDataset, defaultNamespace)
		}
		return fmt.Sprintf("%s-%s-%s", dsType, dsDataset, defaultNamespace)
	}
	...
	return []routeTestCase{
		{
			name: "default",
			otel: false,
			want: renderWantRoute(dsType, defaultDataset, false),
		},
		{
			name: "otel",
			otel: true,
			want: renderWantRoute(dsType, defaultDataset, true),
		},
		{
...
func TestRouteLogRecord(t *testing.T) {
	tests := createRouteTests(defaultTypeLogs)
...
func TestRouteDataPoint(t *testing.T) {
	tests := createRouteTests(defaultTypeMetrics)
...
func TestRouteSpan(t *testing.T) {
	tests := createRouteTests(defaultTypeTraces)
